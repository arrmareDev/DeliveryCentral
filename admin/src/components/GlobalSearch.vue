...
const resultados = ref<{ despachos: ResultItem[]; negocios: ResultItem[]; motorizados: ResultItem[] }>({
    despachos: [], negocios: [], motorizados: [],
})

const totalResultados = computed(() =>
    resultados.value.despachos.length + resultados.value.negocios.length + resultados.value.motorizados.length
)

// Índice plano para navegación con flechas a través de las 3 secciones
function flatIndex(seccion: 'despachos' | 'negocios' | 'motorizados', i: number): number {
    if (seccion === 'despachos') return i
    if (seccion === 'negocios') return resultados.value.despachos.length + i
    return resultados.value.despachos.length + resultados.value.negocios.length + i
}
...
async function buscar() {
    if (query.value.trim().length < 2) {
        resultados.value = { despachos: [], negocios: [], motorizados: [] }
        return
    }
    loading.value = true
    try {
...
function ir(tipo: 'despacho' | 'negocio' | 'motorizado', item: ResultItem) {
    close()
    if (tipo === 'despacho') router.push('/despachos')
    else if (tipo === 'negocio') router.push('/negocios')
...
function onKeydown(e: KeyboardEvent) {
    ...
    if (e.key === 'ArrowDown') {
        ...
        const all = [
            ...
            ...resultados.value.negocios.map(i => ({ tipo: 'negocio' as const, item: i })),
