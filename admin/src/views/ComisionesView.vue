    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 flex flex-wrap items-center gap-2">
            <button v-for="p in PRESETS" :key="p.value" @click="seleccionarPreset(p.value)" class="px-3.5 py-2 rounded-xl text-[12.5px] font-bold cursor-pointer
                       border transition-all duration-150" :class="rango.preset === p.value
            ...
            <div class="flex items-center gap-2 ml-auto">
                <input v-model="rango.desdeInput" type="date" class="px-3 py-2 rounded-xl border border-gray-200 text-[12.5px] text-gray-700
                           outline-none focus:border-red-400 transition-all duration-150" />
                <span class="text-gray-400 text-[12px]">a</span>
                <input v-model="rango.hastaInput" type="date" class="px-3 py-2 rounded-xl border border-gray-200 text-[12.5px] text-gray-700
                           outline-none focus:border-red-400 transition-all duration-150" />
                <button @click="aplicarRangoPersonalizado" :disabled="!rango.desdeInput || !rango.hastaInput" class="px-3.5 py-2 rounded-xl text-[12.5px] font-bold cursor-pointer
                           border-none bg-gray-900 text-white hover:bg-gray-800
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                <p class="text-[10.5px] font-black uppercase tracking-widest text-gray-400 mb-1">
                    Deuda pendiente {{ rango.label }}
                </p>
                <div class="flex items-baseline gap-1">
                    <span class="text-[14px] text-gray-400 font-bold">S/</span>
                    <p class="font-black text-[28px] text-purple-600 leading-none m-0"
                        style="font-family:'Plus Jakarta Sans',sans-serif;">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                <div class="flex items-baseline gap-1">
                    <span class="text-[14px] text-gray-400 font-bold">S/</span>
                    <p class="font-black text-[28px] text-green-600 leading-none m-0"
                        style="font-family:'Plus Jakarta Sans',sans-serif;">
        ...
        <!-- Empty -->
        <div v-else-if="store.resumen.length === 0" class="flex flex-col items-center py-20 text-gray-400 gap-4">
            <div class="w-20 h-20 rounded-2xl bg-gray-100 flex items-center justify-center">
                <BanknotesIcon class="w-10 h-10 text-gray-300" />
            </div>
        ...
        <div v-else class="flex flex-col gap-3">
            <div v-for="m in sortedResumen" :key="m.id" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5
               flex flex-col sm:flex-row sm:items-center gap-4">

                <div class="flex items-center gap-3 flex-1 min-w-0">
                    <div class="w-11 h-11 rounded-full bg-purple-100 flex items-center
                      justify-center text-[14px] font-black text-purple-600 shrink-0">
                        {{ m.nombre.charAt(0).toUpperCase() }}
                    </div>
                    <div class="min-w-0">
                        <p class="font-bold text-[14px] text-gray-900 m-0 truncate">{{ m.nombre }}</p>
                        <p class="text-[12px] text-gray-400 m-0">
                            {{ m.telefono }}
                            <span v-if="m.comisiones_pendientes" class="text-amber-600 font-semibold">
                ...
                <div class="flex items-center gap-6 sm:gap-8">
                    <div>
                        ...
                        <div class="flex items-baseline gap-0.5">
                            ...
                            <span class="font-black text-[18px]"
                                :class="m.deuda_pendiente > 0 ? 'text-purple-600' : 'text-gray-300'">
                    ...
                    <div>
                        ...
                        <div class="flex items-baseline gap-0.5">
                            <span class="text-[11px] text-gray-400">S/</span>
                            <span class="font-black text-[18px] text-green-600">{{ m.total_cobrado.toFixed(2) }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex gap-2 shrink-0">
                   ...
                   text-[12.5px] font-bold border border-gray-200 bg-white text-gray-600
                ...
                <div v-if="detalleModal.show" class="fixed inset-0 z-[500] bg-black/50 backdrop-blur-sm
                 flex items-center justify-center p-4" @click.self="detalleModal.show = false">
                    <Transition enter-active-class="transition-all duration-200 ease-out"
                        ...
                        <div v-if="detalleModal.show" class="w-full max-w-lg bg-white rounded-3xl shadow-2xl p-6
                     max-h-[85vh] flex flex-col">
                            ...
                            <div class="flex items-center justify-between mb-4 shrink-0">
                                ...
                                <button @click="detalleModal.show = false" class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center
                         cursor-pointer border-none hover:bg-gray-200 transition-colors">
                            ...
                            <div class="flex-1 overflow-y-auto -mx-2 px-2">
                                ...
                                <div v-else-if="store.detalle.length === 0"
                                    class="flex flex-col items-center py-10 text-gray-400 gap-2">
                                ...
                                <div v-else class="flex flex-col gap-2">
                                    <div v-for="c in store.detalle" :key="c.id"
                                        ...
                                        <div class="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                                            :class="c.estado === 'pendiente' ? 'bg-amber-50' : 'bg-green-50'">
                                            <component :is="c.estado === 'pendiente' ? ClockIcon : CheckCircleIcon"
                                                class="w-4.5 h-4.5"
                                                :class="c.estado === 'pendiente' ? 'text-amber-500' : 'text-green-500'" />
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <p class="font-semibold text-[12.5px] text-gray-900 m-0">
                                        ...
                                        <div class="flex items-baseline gap-0.5 shrink-0">
                                            ...
                                            <span class="font-black text-[14px]"
                                                :class="c.estado === 'pendiente' ? 'text-purple-600' : 'text-green-600'">
                                                {{ c.monto.toFixed(2) }}
                                            </span>
                                        </div>
                                    </div>
                            ...
                            <div v-if="store.deudaPendiente > 0" class="pt-4 mt-2 border-t border-gray-100 shrink-0">
...
// ── Modal detalle ──────────────────────────────────────────
const detalleModal = reactive({ show: false, motorizadoId: 0 })

async function openDetalle(id: number) {
    detalleModal.motorizadoId = id
    detalleModal.show = true
    await store.fetchDetalle(id, paramsRango())
}
...
async function executeCobro() {
    ...
    const result = await store.cobrar(confirmCobro.motorizadoId, paramsRango())
